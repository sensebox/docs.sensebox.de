...
Nachfolgend geben wir ein Intervall an, in welchen die Daten an das TTN-Netzwerk geschickt werden sollen. In diesem Sketch beträgt das Intervall alle 60 Sekunden. Des Weiteren werden die Pins gemappt und eine `onEvent()`-Funktion zum Debuggen im Seriellen Monitor wird erstellt.
...
Als nächstes definieren wir das LoRa Paket, was an das TTN-Netzwerk übermittelt wird. Mit `LoRaMessage message` deklarieren wir das Paket `message`, in dem sukzessiv die Messwerte unserer Sensoren hinzugefügt werden.
...
Anschließend folgt die `setup()`-Funktion, die du es aus den bisherigen Arduino-Sketches bereits kennengelernt hast. In dieser starten wir unsere Sensoren und den seriellen Monitor.
...
Zu guter Letzt fehlt noch die `loop()`-Funktion. In dieser wird deklariert, dass der in den globalen Variablen definierte `os_loop()` ausgeführt wird.
